...
class PrototypeMixtureGenerator(nn.Module):
    ...
    def forward(self, context: torch.Tensor, prototype_bank: torch.Tensor) -> torch.Tensor:
        if prototype_bank.ndim != 2:
            raise ValueError(
                f"prototype_bank must have shape [prototype_count, feature_dim], "
                f"got {tuple(prototype_bank.shape)}"
            )
        if prototype_bank.size(0) != self.prototype_count:
            raise ValueError(
                "Prototype count mismatch: generator was initialized for "
                f"{self.prototype_count} prototypes, but the bank contains "
                f"{prototype_bank.size(0)} prototypes"
            )
