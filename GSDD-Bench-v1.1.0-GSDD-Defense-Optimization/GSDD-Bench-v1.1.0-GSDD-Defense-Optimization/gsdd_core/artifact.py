...
def normalize_attack_bundle(bundle: Mapping[str, object]) -> dict[str, object]:
    ...
    previous_label_count = out.get("label_num_nodes_before_padding", label_num_nodes)
    try:
        previous_label_count = int(previous_label_count)
    except (TypeError, ValueError):
        previous_label_count = label_num_nodes
    out["label_num_nodes_before_padding"] = min(previous_label_count, poison_num_nodes)
    out["injected_node_idx"] = torch.nonzero(poison_y < 0, as_tuple=False).reshape(-1).long()
