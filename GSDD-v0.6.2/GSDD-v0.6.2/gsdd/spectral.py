...
def decompose_delta_gain(delta_gain: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Split cross-model log-gain difference into scale and shape.

    ``delta_gain`` has shape [N, B]. Adding the same scalar to every band can
    arise from an arbitrary representation rescaling and is therefore not a
    spectral-shape effect. The per-node mean records that scale level, while
    centering across bands produces a multiplicative-scale-invariant profile.
    """
    if delta_gain.ndim != 2:
        raise ValueError("delta_gain must have shape [N, B]")
    level = delta_gain.mean(dim=1, keepdim=True)
    shape = delta_gain - level
    shape_norm = torch.linalg.vector_norm(shape, ord=2, dim=1, keepdim=True)
    return level, shape, shape_norm
