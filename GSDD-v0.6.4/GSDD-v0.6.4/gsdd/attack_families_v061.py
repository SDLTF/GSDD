...
def _optimize_generator_round(
    ...
    for epoch in range(1, cfg.attack.generator_epochs + 1):
        ...
        for name, component in {**binding, **stealth}.items():
...
def build_attack_plan_v061(
    clean_graph: GraphData,
    cfg: Config,
    device: torch.device,
) -> AttackPlan:
    family = cfg.attack.family.lower().strip()
    if family not in ATTACK_FAMILIES_V061:
        raise ValueError(
            f"attack.family must be one of {ATTACK_FAMILIES_V061}, got {family!r}"
        )
    ...
    if family == "dpgba_style_binding_aware":
        # Use a class-agnostic prototype bank. A target-class-only bank directly
        # encodes target semantics and can attack clean/label-only models.
        prototypes = clean_graph.x[clean_graph.train_mask]
        ...
        prototype_bank = prototypes[positions].to(device)
