...
def build_attack_plan_v064(
    clean_graph: GraphData,
    cfg: Config,
    device: torch.device,
) -> AttackPlan:
    family = cfg.attack.family.lower().strip()
    if family not in ATTACK_FAMILIES_V064:
        raise ValueError(
            f"attack.family must be one of {ATTACK_FAMILIES_V064}, got {family!r}"
        )
