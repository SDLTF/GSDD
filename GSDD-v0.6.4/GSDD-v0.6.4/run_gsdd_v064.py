...
from gsdd.attack_families_v064 import build_attack_plan_v064
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="GSDD-v0.6.4 clean-label model x trigger factorial audit"
    )
...
def apply_overrides(cfg: Config, args: argparse.Namespace) -> Config:
    ...
    cfg._allow_cpu_v064 = bool(args.allow_cpu)  # type: ignore[attr-defined]
    return cfg
...
def run(cfg: Config, source_config: Path) -> Path:
    allow_cpu = bool(getattr(cfg, "_allow_cpu_v064", False))
    if not allow_cpu and not cfg.experiment.device.lower().startswith("cuda"):
        raise RuntimeError("GSDD-v0.6.4 formal experiments are CUDA-only")
    if cfg.experiment.device.lower().startswith("cuda") and not torch.cuda.is_available():
        raise RuntimeError(
            "GSDD-v0.6.4 requires CUDA for formal experiments, but "
            "torch.cuda.is_available() is False"
        )
    if cfg.attack.selection_method != "clean_label":
        raise ValueError("GSDD-v0.6.4 only audits clean-label attacks")
    ...
    summary: dict[str, Any] = {
        ...
        "protocol": "dual_clean_label_model_by_trigger_factorial",
        "attack_mode": str(cfg.attack.clean_label_attack_mode),
    }
    save_json(summary, run_dir / "summary.json")
    log(f"[GSDD-v0.6.4] run_dir={run_dir}", cfg.output.verbose)
    ...
    try:
        ...
        plan = build_attack_plan_v064(clean_graph, cfg, device)
        if plan.family != "dpgba_style_binding_aware":
            raise AssertionError("v0.6.4 formal protocol expects DPGBA-style plan")
        ...
        log(
            f"[Done:v0.6.4] validity={attack_validity['status']} "
