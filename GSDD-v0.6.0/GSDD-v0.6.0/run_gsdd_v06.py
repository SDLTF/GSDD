...
from gsdd.attack_families import (
    ATTACK_FAMILIES,
    build_attack_plan,
    build_paired_graphs,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="GSDD-v0.6 CUDA-only attack-family generalization audit"
    )
    parser.add_argument("--config", default="configs/cora_attack_generalization.yaml")
    parser.add_argument("--seed", type=int, default=None)
    parser.add_argument("--device", default=None, help="Only cuda/cuda:0 are accepted in v0.6")
    parser.add_argument("--attack-family", choices=ATTACK_FAMILIES, default=None)
...
def apply_overrides(cfg: Config, args: argparse.Namespace) -> Config:
    ...
    if args.device is not None and not args.device.lower().startswith("cuda"):
        raise ValueError("GSDD-v0.6 is CUDA-only; --device must start with 'cuda'")
...
def make_summary_markdown(summary: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        "# GSDD-v0.6 Attack-Family Generalization Summary",
        "",
...
def run(cfg: Config, source_config: Path) -> Path:
    if cfg.experiment.device.lower() != "cuda" and not cfg.experiment.device.lower().startswith("cuda:"):
        raise RuntimeError("GSDD-v0.6 formal experiments are CUDA-only")
    if not torch.cuda.is_available():
        raise RuntimeError(
            "GSDD-v0.6 requires CUDA, but torch.cuda.is_available() is False. "
    ...
    log(f"[GSDD-v0.6] run_dir={run_dir}", cfg.output.verbose)
    log(f"[GSDD-v0.6] device={device} attack_family={cfg.attack.family}", cfg.output.verbose)
    ...
    try:
        ...
        attack_plan = build_attack_plan(clean_graph, cfg, device)
        ...
        log(
            f"[Done:{attack_plan.family}] full_ASR={behavior['full']['triggered_asr']:.4f} "
