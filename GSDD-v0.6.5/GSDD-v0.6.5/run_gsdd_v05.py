...
from gsdd.data import GraphData, load_dataset, make_triggered_test_graph, poison_training_graph
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="GSDD-v0.5 paired backdoor-specific spectral difference-in-differences audit"
    )
    parser.add_argument("--config", default="configs/cora_paired_did.yaml")
...
def build_graphs(clean_graph: GraphData, cfg: Config) -> tuple[dict[str, GraphData], torch.Tensor]:
    graphs: dict[str, GraphData] = {}
    victims_ref: torch.Tensor | None = None
    for mode in MODES:
        graph, victims = poison_training_graph(
            clean_graph,
        ...
        if victims_ref is None:
            victims_ref = victims
        elif not torch.equal(victims_ref, victims):
            raise AssertionError("Victim selection differs across paired intervention modes")
        graphs[mode] = graph

    assert victims_ref is not None
    # The within-pair graphs must be exactly equal except for training labels.
...
def make_summary_markdown(summary: dict[str, Any], path: Path) -> None:
    ...
    lines = [
        "# GSDD-v0.5 Paired Backdoor-Specific DID Summary",
        "",
        "## Run status",
        "",
...
def run(cfg: Config, source_config: Path) -> Path:
    ...
    log(f"[GSDD-v0.5] run_dir={run_dir}", cfg.output.verbose)
    log(f"[GSDD-v0.5] device={device}", cfg.output.verbose)
    ...
    try:
        ...
        graphs, victims = build_graphs(clean_graph, cfg)
        log(
        ...
        log(
            f"[Done] full_ASR={behavior['full']['triggered_asr']:.4f} "
