...
class AttackConfig:
    ...
    # v0.6.5 generic selective-activation repair. These losses are evaluated
    # only on held-out non-target calibration nodes, preventing target-class
    # clean-label victims from making the clean-model penalty ill-posed.
    generic_clean_cap_weight: float = 12.0
    generic_clean_probability_cap: float = 0.15
    generic_selectivity_weight: float = 8.0
    generic_selectivity_margin: float = 0.40
    generic_target_similarity_weight: float = 1.5
    generic_target_similarity_allowance: float = 0.05
