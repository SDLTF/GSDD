...
from .spectral import band_energies, decompose_delta_gain, js_divergence, log_band_gain
...
def zero_sum_basis(size: int) -> np.ndarray:
    """Return an orthonormal basis for vectors whose coordinates sum to zero."""
    if size < 2:
        raise ValueError("size must be at least 2")
    centered = np.eye(size, dtype=np.float64) - np.ones((size, size), dtype=np.float64) / size
    basis, _ = np.linalg.qr(centered[:, : size - 1])
    return basis[:, : size - 1]


def robust_shape_mahalanobis(values: np.ndarray, epsilon: float) -> np.ndarray:
    """Label-free multivariate anomaly score for transfer-shape coordinates.

    The centered log-gain profile is first standardized by coordinate-wise
    median/MAD. A robust minimum-covariance estimate is then used when
    possible; Ledoit-Wolf shrinkage is the deterministic fallback. Unlike the
    v0.3 coordinate-wise ECDF average, this score respects covariance and
    measures distance in the joint zero-sum spectral-shape geometry.
    """
    values = _as_matrix(values)
    ...
    try:
        if n_samples >= max(20, 3 * n_features):
            estimator = MinCovDet(
        ...
        squared = estimator.mahalanobis(standardized)
    except Exception:
        estimator = LedoitWolf(assume_centered=False).fit(standardized)
        squared = estimator.mahalanobis(standardized)
...
def _save_centered_band_profile(
    centered_delta_layers: list[np.ndarray],
    y_true: np.ndarray,
    output_dir: Path,
) -> None:
    for layer_index, delta in enumerate(centered_delta_layers, start=1):
        clean_mean = delta[y_true == 0].mean(axis=0)
        poison_mean = delta[y_true == 1].mean(axis=0)
        x_axis = np.arange(delta.shape[1])
        width = 0.38
        ...
        plt.xlabel("Bernstein graph-frequency band")
        plt.ylabel("Centered supervised-minus-SSL log gain")
        plt.title(f"Scale-invariant transfer shape, layer {layer_index}")
        plt.legend()
        plt.tight_layout()
        plt.savefig(output_dir / f"band_transfer_shape_layer{layer_index}.png", dpi=180)
        plt.close()
...
def compute_diagnostics(
    ...
    delta_gain_level_layers: list[torch.Tensor] = []
    delta_gain_shape_layers: list[torch.Tensor] = []
    delta_gain_shape_norm_layers: list[torch.Tensor] = []
    ...
    for supervised_hidden_layer, ssl_hidden_layer in zip(supervised_hidden, ssl_hidden):
        ...
        delta_gain = supervised_gain - ssl_gain
        delta_gain_layers.append(delta_gain)
        gain_level, gain_shape, gain_shape_norm = decompose_delta_gain(delta_gain)
        delta_gain_level_layers.append(gain_level)
        delta_gain_shape_layers.append(gain_shape)
        delta_gain_shape_norm_layers.append(gain_shape_norm)
    ...
    delta_gain_level_np = (
        torch.cat(delta_gain_level_layers, dim=1)[candidate_indices].detach().cpu().numpy()
    )
    delta_gain_shape_np = (
        torch.cat(delta_gain_shape_layers, dim=1)[candidate_indices].detach().cpu().numpy()
    )
    delta_gain_shape_norm_np = (
        torch.cat(delta_gain_shape_norm_layers, dim=1)[candidate_indices].detach().cpu().numpy()
    )

    # Project every layer's centered B-band profile into the (B-1)-dimensional
    # zero-sum subspace, then score the concatenated joint geometry.
    shape_basis = zero_sum_basis(num_bands)
    shape_geometry_np = np.concatenate(
        [
            layer[candidate_indices].detach().cpu().numpy() @ shape_basis
            for layer in delta_gain_shape_layers
        ],
        axis=1,
    )
    shape_geometry_mahalanobis = robust_shape_mahalanobis(
        shape_geometry_np,
        epsilon=mad_epsilon,
    )
    relation_layers = []
    ...
    # The original five components are retained exactly for v0.2 comparability.
    primary_raw_components: dict[str, np.ndarray] = {
    ...
    # v0.3 decomposes H4 into a scale-sensitive level and a scale-invariant
    # spectral shape. Only the latter can support a genuine frequency-transfer
    # claim because hidden representations can otherwise be rescaled freely.
    audit_raw_components: dict[str, np.ndarray] = {
        "transfer_level": delta_gain_level_np,
        "transfer_shape": delta_gain_shape_np,
        "transfer_shape_norm": delta_gain_shape_norm_np,
    }
    raw_components = {**primary_raw_components, **audit_raw_components}
    ...
    legacy_component_matrix = np.stack(
        [legacy_components[name] for name in primary_raw_components], axis=1
    )
    ...
    def select_family(family: dict[str, np.ndarray], names: list[str]) -> dict[str, np.ndarray]:
        return {name: family[name] for name in names}

    primary_names = list(primary_raw_components)
    scale_invariant_names = ["input_spectrum", "model_js", "transfer_shape", "transfer_shape_norm"]
    combined_scores = {
        "legacy_combined": legacy_combined,
        "global_mad_combined": combine_components(select_family(global_mad_components, primary_names)),
        "global_ecdf_combined": combine_components(select_family(global_ecdf_components, primary_names)),
        "trimmed_combined": combine_components(select_family(trimmed_components, primary_names)),
        "hybrid_combined": combine_components(select_family(hybrid_components, primary_names)),
        "global_ecdf_scale_invariant": combine_components(
            select_family(global_ecdf_components, scale_invariant_names)
        ),
        "hybrid_scale_invariant": combine_components(
            select_family(hybrid_components, scale_invariant_names)
        ),
    }

    # Direct multivariate shape score: no poison labels and no oracle direction.
    combined_scores["shape_geometry_mahalanobis"] = shape_geometry_mahalanobis
    ...
    for coordinate in range(shape_geometry_np.shape[1]):
        frame_data[f"shape_geometry_{coordinate}"] = shape_geometry_np[:, coordinate]
    ...
    delta_gain_shape_candidates = []
    for layer, (delta, level, shape, shape_norm) in enumerate(
        zip(
            delta_gain_layers,
            delta_gain_level_layers,
            delta_gain_shape_layers,
            delta_gain_shape_norm_layers,
        ),
        start=1,
    ):
        delta_np = delta[candidate_indices].detach().cpu().numpy()
        level_np = level[candidate_indices].detach().cpu().numpy()[:, 0]
        shape_np = shape[candidate_indices].detach().cpu().numpy()
        shape_norm_np = shape_norm[candidate_indices].detach().cpu().numpy()[:, 0]
        delta_gain_candidates.append(delta_np)
        delta_gain_shape_candidates.append(shape_np)
        frame[f"gain_level_l{layer}"] = level_np
        frame[f"gain_shape_norm_l{layer}"] = shape_norm_np
        for band in range(num_bands):
            frame[f"delta_gain_l{layer}_b{band}"] = delta_np[:, band]
            frame[f"centered_delta_gain_l{layer}_b{band}"] = shape_np[:, band]
    ...
    primitive_groups: dict[str, dict[str, np.ndarray]] = {
        ...
        "transfer_level": {
            f"gain_level_l{layer}": delta_gain_level_np[:, layer - 1]
            for layer in range(1, delta_gain_level_np.shape[1] + 1)
        },
        "transfer_shape": {
            f"centered_delta_gain_l{layer}_b{band}": delta_gain_shape_candidates[layer - 1][:, band]
            for layer in range(1, len(delta_gain_shape_candidates) + 1)
            for band in range(num_bands)
        },
        "transfer_shape_norm": {
            f"gain_shape_norm_l{layer}": delta_gain_shape_norm_np[:, layer - 1]
            for layer in range(1, delta_gain_shape_norm_np.shape[1] + 1)
        },
        "transfer_shape_geometry": {
            f"shape_geometry_{coordinate}": shape_geometry_np[:, coordinate]
            for coordinate in range(shape_geometry_np.shape[1])
        },
    }
    ...
    if make_plots and len(np.unique(y_true)) == 2:
        ...
        _save_centered_band_profile(delta_gain_shape_candidates, y_true, output_dir)
    ...
    extra = {
        ...
        "delta_gain_shape_layers": delta_gain_shape_candidates,
